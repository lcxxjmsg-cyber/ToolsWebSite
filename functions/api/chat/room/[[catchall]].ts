...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  // POST /api/chat/room → pathParts = ['api', 'chat', 'room'] (length 3)
  if (pathParts.length < 3 || pathParts[0] !== 'api' || pathParts[1] !== 'chat' || pathParts[2] !== 'room') {
  ...
  // --- Route dispatching ---
  // rest = []                    → POST /api/chat/room (create)
  // rest = [roomId]              → GET/DELETE /api/chat/room/:id
  // rest = [roomId, 'msg']       → GET/POST /api/chat/room/:id/msg
  ...
  if (rest.length === 0) {
    ...
    const ttl = body.expiresIn && body.expiresIn > 0 ? Math.min(body.expiresIn, ROOM_TTL_MAX) : ROOM_TTL_MAX;
  ...
  // /api/chat/room/:id (single segment after /room)
  if (rest.length === 1) {
  ...
  // /api/chat/room/:id/msg
  if (rest.length === 2 && rest[1] === 'msg') {
    if (request.method === 'POST') {
